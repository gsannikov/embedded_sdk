...
# AutoForge imports
from auto_forge import (AutoForgeModuleType, CoreModuleInterface, CoreRegistry, AIModelType, AIProvidersType,
...
class CoreAIBridge(CoreModuleInterface):
    ...
    def _initialize(self) -> None:
        ...
        self._logger.info(
            f"AI Bridge successfully initialized with provider '{self._provider.pretty_name}', model: '{self._provider.model.value}'")
        self._enabled = True
    ...
    def _load_providers(self, joker: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not isinstance(self._provider.model, AIModelType) or self._provider.model == AIModelType.UNKNOWN:
                self._logger.error(f"'model' in provider {self._provider.name} is missing or not supported")
                return False
    ...
    @staticmethod
    def _estimate_token_count(prompt: str, context: str = "", ai_type: AIModelType = AIModelType.GPT_4) -> int:
        full_text = context + prompt

        with contextlib.suppress(Exception):
            if "GPT" in ai_type.name:
                enc = tiktoken.encoding_for_model(ai_type.value)
    ...
    def _prepare_request(self,
        ...
        Args:
            ...
            context (str): A system prompt defining the assistant's behavior.
            max_tokens (int): The maximum number of tokens to be generated by the model,overrides the provider 'max_tokens' value.
        ...
        # Use 'provider.max_tokens' when max_tokens was not specified.
        if not isinstance(max_tokens, int):
            if isinstance(self._provider.max_tokens, int):
                max_tokens = self._provider.max_tokens

        # Estimate tokens count (for known IA models)
        estimated_tokens = self._estimate_token_count(prompt=prompt, context=context)
        if isinstance(max_tokens, int) and estimated_tokens > max_tokens:
            self._logger.warning(f"Estimated token count ({estimated_tokens}) exceeds max_tokens limit ({max_tokens})")
        ...
        if self._provider.name == "openai":
            ...
            self._payload = {
                "model": self._provider.model.value,
                "messages": [
        ...
        # ----------------------------------------------------------------------
            ...
            self._payload = {
                ...
                "max_completion_tokens": self._provider.max_tokens if isinstance(self._provider.max_tokens,
                                                                                 int) is not None else 10000
            }
    ...
    async def query(self, prompt: str, context: Optional[str] = None, max_tokens: Optional[int] = None,
        ...
        try:
            ...
            # Prepare the request
            self._prepare_request(prompt=prompt, context=request_context, max_tokens=max_tokens,
